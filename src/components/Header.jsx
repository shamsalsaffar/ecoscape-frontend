...
const Header = ({ onFetch }) => {
  ...
  const handleClick = () => { // kan använda denna function för become a host i framtiden
  ...
  return (
    <header className="header">
      <h1>ECOSCAPAE</h1>
      <nav style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 1rem", width: "100%" }}>
  {/* Vänstra länkar */}
  <div style={{ display: "flex", gap: "1.5rem" }}>
    <Link
      to="/Home"
      onClick={onFetch}
      style={{ color: "white", cursor: "pointer", textDecoration: "none", marginLeft: "2rem" }}
    >
      Home
    </Link>
  ...
  <div style={{ display: "flex", alignItems: "center", gap: "0.2rem" }}>
    <Button onClick={handleClick} className="icon-button" style={{ padding: "0.5rem 1rem", borderRadius: "none" }}>
      Become A Host
    </Button>
    <Button onClick={toggleDropdown} className="icon-button" style={{ padding: "0.5rem", borderRadius: "none" }}>
